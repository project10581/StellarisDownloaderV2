...
## 18. Workshop 覆盖层与列表右键菜单验收修正

已完成：

- 真实 WebView2 截图发现，同一张 Steam Workshop 卡片通常同时包含缩略图、标题和详情动作等多个指向同一 ID 的链接；原注入脚本逐个链接附加按钮，未命中新版卡片 class 时又退回各自的父元素，因此标题链接会得到第二个按钮。标题行的固定高度与裁剪规则把该按钮显示成遮挡文字的蓝色或绿色短条。
- 注入脚本现在只把具有足够尺寸的缩略图媒体链接视为视觉目标，并以目标元素为键协调按钮生命周期；标题、作者和小图标链接不会注入。按钮位于 `body` 下的独立 Shadow DOM 覆盖层，通过可见矩形定位，不再写入 Steam 卡片或修改页面容器的 `position`。滚动、尺寸变化、动态 DOM、图片加载和轮播位移会触发合并后的重定位；目标消失时按钮同步移除。
- 重复注入现在会完整释放旧 MutationObserver、定时器、动画帧、事件监听器和覆盖层，并持续清理旧版本遗留在页面 DOM 内的按钮。状态没有变化时不再重复改写按钮内容；点击处理器读取按钮当前的 Workshop ID，避免 DOM 复用后继续提交旧 ID。
- 列表右键菜单的“刷新列表”和“重新扫描模组库”原先使用独立 `Click` 处理器，绕过了顶部栏 `RefreshCommand` / `RescanCommand` 的 `CanExecute`。菜单项现在经 `ContextMenu.PlacementTarget.DataContext` 直接绑定同一命令实例，因此执行路径、忙碌保护和可用状态与顶部栏一致；其余选择相关菜单项也显式绑定对应的可用状态。
